    ...
    <aside className="bg-secondary opacity-0 animate-fadeRight hover:border-accent border-borders flex max-h-[96dvh] min-h-[95dvh] w-[95dvw] flex-col justify-center gap-20 sm:gap-25 rounded-4xl border-2 p-8 sm:p-30 lg:p-8 transition-colors duration-700 ease-in-out lg:fixed lg:max-h-[96dvh] lg:min-h-[95dvh] lg:w-[32rem] lg:gap-10 xl:gap-20">
      <div className="flex flex-col items-center gap-8">
        ...
        <figure className="flex items-center justify-center">
          {/* profile  */}
          <Image
            className="border-borders sm:w-[25rem] sm:h-[25rem] lg:w-[18rem] lg:h-[18rem] rounded-4xl border-4"
            src={profile}
            width={180}
            height={180}
            priority
        ...
        <article className="flex flex-col gap-7 text-center">
          {/* info  */}
          <h1 className="font-poppins text-text text-[2.4rem] lg:text-[2.4rem] sm:text-[3rem] font-bold">{Profile.NAME}</h1>
          <div className="flex flex-col gap-1">
            <p className="text-accent font-mono text-[1.3rem] lg:text-[1.3rem] sm:text-[1.8rem]">{Profile.INTRO}</p>
            <p className="text-accent font-mono text-[1.3rem] lg:text-[1.3rem] sm:text-[1.8rem]">
              Founder{' '}
              <a
                href={Profile.LINK_1}
          ...
          {Profile.SOCIALS.map((social) => {
            ...
            return (
              <Link key={social.id} href={social.href} target="blank">
                <figure className="bg-tertiary shadow-custom flex size-16 sm:size-20 lg:size-16 items-center justify-center rounded-2xl transition-all duration-300 ease-in-out hover:scale-105">
